## 把ts打包成js
* 把ts打包成js需要使用gulp命令。
* 使用gulp命令需要`gulp-cli`包和`gulp`包和`gulp-babel`包和`@babel/preset-typescript`包和`@babel/preset-env`包（包含`@babel/plugin-transform-modules-commonjs`包和`@babel/core`包）和`gulpfile.ts`配置文件。
